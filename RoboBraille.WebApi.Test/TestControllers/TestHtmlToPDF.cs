...
namespace RoboBraille.WebApi.Test
{
    ...
    {
        [TestMethod]
        public async Task TestPostHtmlToPDF()
        {
            //init
            ...
            HTMLtoPDFJob hpj = new HTMLtoPDFJob()
            {
                Id = Guid.NewGuid(),
                FileContent = File.ReadAllBytes(IOController.GetInputFiles().Where(x => x.Key.EndsWith("A3.testHTMLtable.html")).Select(x => x.Key).First()),
                UserId = Guid.Parse("d2b97532-e8c5-e411-8270-f0def103cfd0"),
                FileExtension = ".pdf",
                FileName = "test",
            ...
            HTMLtoPDFJob hpj2 = new HTMLtoPDFJob()
            {
                Id = Guid.NewGuid(),
            ...
            var jobs = new List<HTMLtoPDFJob> { hpj2 }.AsQueryable();
            ...
            var repo = new HTMLtoPDFRepository(mockContext.Object);
            ...
            var serviceController = new HTMLtoPDFController(repo);
            serviceController.Request = request;
            //call
            await serviceController.Post(hpj);
            //test
