app.factory('helperService', [function () {
    var helperService = {};
    helperService.createAuthorizationHeader = function (uploadUrl, method) {
        var credentials = {
    ...
    helperService.localeCodeToEnglish = function (loc) {
...
app.factory('httpService', ['$http', '$q', 'helperService', 'poller', function ($http, $q, helperService, poller) {
    var httpService = {};


    httpService.InitInputParameters = function (url) {
        var roboAppOptions = {};
        roboAppOptions.acc = [];
        roboAppOptions.langList = [];
        roboAppOptions.brlcont = [];
        roboAppOptions.brlLangList = [];
        roboAppOptions.brlMathList = [];
        roboAppOptions.brlMathList.push({ value: -1, name: 'None' });
        roboAppOptions.daisyLangList = [];
        var deferred = $q.defer();
        $http.get(url + '/api/AccessibleConversion/GetOutputFormats').then(function (result) {
            for (var f in result.data) {
                var item = { value: result.data[f], name: result.data[f].substr(4, result.data[f].length) };
                roboAppOptions.acc.push(item);
            }
            $http.get(apiPaths['audio'] + '/GetLangs').then(function (result) {
                for (var l in result.data) {
                    var item = { code: result.data[l], name: helperService.localeCodeToEnglish(result.data[l].substr(0, 2) + '-' + result.data[l].substr(2)) };
                    roboAppOptions.langList.push(item);
                }
                $http.get(apiPaths['brailleMath'] + '/getmathformats').then(function (result) {
                    for (var m in result.data) {
                        var i = 1;
                        var item = { value: i, name: result.data[m] }
                        i++;
                        roboAppOptions.brlMathList.push(item);
                    }
                    $http.get(apiPaths['braille'] + '/GetContractions').then(function (result) {
                        var i = 1;
                        for (var c in result.data) {
                            var item = { id: i, name: result.data[c] };
                            roboAppOptions.brlcont.push(item);
                            i++;
                        }
                        var arrB = ['enUS', 'enGB', 'bgBG', 'daDK', 'frFR', 'deDE', 'elGR', 'huHU', 'isIS', 'itIT', 'noNN', 'plPL', 'ptPT', 'roRO', 'slSI', 'esES'];
                        for (var b in arrB) {
                            var item = { code: arrB[b], name: helperService.localeCodeToEnglish(arrB[b].substr(0, 2) + '-' + arrB[b].substr(2)) }
                            roboAppOptions.brlLangList.push(item);
                        }
                        var arrD = ['bgBG', 'daDK', 'enUS', 'deDE', 'huHU', 'plPL', 'roRO'];
                        for (var d in arrD) {
                            var item = { code: arrD[d], name: helperService.localeCodeToEnglish(arrD[d].substr(0, 2) + '-' + arrD[d].substr(2)) }
                            roboAppOptions.daisyLangList.push(item);
                        }
                        deferred.resolve(roboAppOptions);
                    });
                });
            });
        });
    ...
    httpService.PostJobToUrl = function (uploadUrl, params) {
        var fd = new FormData();
        for (var key in params) {
            fd.append(key, params[key]);
        }
        var deferred = $q.defer();
        var header = helperService.createAuthorizationHeader(uploadUrl, 'POST');
        if (header == null) {
            throw new Error("Something went badly wrong with creating the header!");
        }
        ...
        }).catch(function (data, status) { }).finally(function () { });
        return deferred.promise;
    }

    httpService.JobStatus = function (url, id) {
        var deferred = $q.defer();
        var count = 0;
        var header = helperService.createAuthorizationHeader(url + '?jobId=' + id, 'GET');
        var myPoller = poller.get(url, {
            action: 'get',
            delay: 10000,
            argumentsArray: [
                {
    ...
    httpService.JobGetRequest = function (url, id) {
        var deferred = $q.defer();
        var header = helperService.createAuthorizationHeader(url + '?jobId=' + id, 'GET');
        $http.get(url, {
            //headers: {
    ...
    httpService.getUrlContent = function (url) {
        var deferred = $q.defer();
        //solve CORS problem
